  ...
  for i in range(nall):
    ...
    for survey in surveys:
        ...
        if survey[:3]+str(i)!=lastsurvey:
            ...
            S[survey].stochasticObserving(mode="1P",SOdraw=SOdraw)
            print "1p warning"
            if S[survey].seeingtest=="Fail":
                lenspars["pf"][survey]={}
                for src in S[survey].sourcenumbers:
        ...
        if pf[1]:
            folder="forPhil"
            folder="%s/%i"%(folder,count)         

            bands=["g_SDSS","r_SDSS","i_SDSS"]
            import colorImage
            color = colorImage.ColorImage()
            color.bMinusr=(S[survey].ml[bands[0]]-S[survey].ml[bands[2]])/4.
            color.bMinusg=(S[survey].ml[bands[0]]-S[survey].ml[bands[1]])/4.
            color.nonlin=4.
            colorimage = color.createModel(\
                 S[survey].image[bands[0]],S[survey].image[bands[1]],S[survey].image[bands[2]])
            colorresid=color.colorize(\
                 S[survey].fakeResidual[0][bands[0]],\
                 S[survey].fakeResidual[0][bands[1]],\
                 S[survey].fakeResidual[0][bands[2]])
            plt.imshow(colorimage,interpolation=None)
            plt.savefig(folder+"_colour.png")
            plt.show()
            plt.imshow(colorresid,interpolation=None)
            plt.savefig(folder+"_colourresid.png")
            plt.show()
            count+=1
