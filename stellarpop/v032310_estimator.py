...
class Estimator(NestedSampler):
    ...
    def mcmc(self,outname,niter,nburn=0,nthin=1):
        ...
        delay = nburn/2
        interval = nburn/4+1
        #sampler.use_step_method(MySM.MWAdaptiveMetropolis,mcmcpars.values(),delay=delay,interval=interval,greedy=False,doLikelihood=True)
        ...
        for key in self.names:
            mcmcpars[key].value = sampler.trace(key)[:][-1]
